...
//    ret = idev->read(ori, len);
    ret = ilitek_thp_send_cmd(cmd, core_mp.cdc_len, ori, len);
    if (ret < 0) {
...
{
    struct mp_test_P540_open open[2];//mp_test_P540_open open[tItems[index].frame_count] wenjie
...
{
    struct mp_test_open_c open[2];//open[tItems[index].frame_count] wenjie
    int i = 0, x = 0, y = 0, ret = 0, addr = 0;
    char str[512] = {0};
...
//#ifdef ODM_HQ_EDIT
/* dongfeiju, 2020/06/03, Add for ITO test name*/
    if (idev->tp_type == TP_BOE) {
        snprintf(tp_module, 32, "%s", "BOE");
    } else if (idev->tp_type == TP_HLT) {
       snprintf(tp_module, 32, "%s", "HLT");
    }
//#endif
    if (core_mp.final_result == MP_FAIL)
