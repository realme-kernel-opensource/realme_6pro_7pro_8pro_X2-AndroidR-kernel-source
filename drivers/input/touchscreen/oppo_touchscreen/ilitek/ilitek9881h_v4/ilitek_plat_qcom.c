...
static bool ilitek_irq_throw_away(void *chip_data)
{
    ...
    if (chip_info->need_judge_irq_throw) {
        ...
        TPD_INFO("wake_up_interruptible !\n");
        return true;
                                  ...
                                  u8 read_type)
{
    ...
    if (idev->write(pre_temp, sizeof(pre_temp)) < 0) {
        TPD_INFO("write pre 0xF6 0x02 cmd fail\n");
    }
    ...
    ipio_err("debug cmd 0x%X, 0x%X", test_cmd[0], test_cmd[1]);
    test_cmd[0] = 0xFA;
    test_cmd[1] = read_type;
    if (idev->write(test_cmd, 2) < 0) {
        TPD_INFO("write 0xFA  0x08 or 0x03 cmd failed ret = %d\n", ret);
    }
    ...
    if (idev->read(buf, rlen) < 0) {
        TPD_INFO("Read data failed ret = %d\n", ret);
    }
...
//#endif
    ...
    if (ret < 0) {
        TPD_INFO("register common touch device failed\n");
