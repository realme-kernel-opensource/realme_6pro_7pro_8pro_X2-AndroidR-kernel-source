...
static int host_download_dma_check(u32 start_addr, u32 block_size, u32 *dma_crc)
{
    ...
    /* Dma1 stop */
    ilitek_ice_mode_write(0x072100, 0x02000000, 4);
...
static int ilitek_tddi_fw_iram_program(u32 start, u8 *w_buf, u32 w_len, u32 split_len)
{
	...
	if (split_len != 0) {
		for (addr = start, i = 0; addr < end; addr += split_len, i += split_len) {
			...
			if (((addr + split_len) >= end) && ((split_len % 4) != 0)) {
				TPD_INFO("org split_len = 0x%X split_len %% 4 = %d\n", split_len, split_len % 4);
				TPD_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n", split_len - 4, idev->update_buf[5 + split_len - 4]);
		...
		if (w_len % 4 != 0) {
			TPD_INFO("org w_len = 0x%X split_len %% 4 = %d\n", w_len, w_len % 4);
			w_len = w_len + (4 - (w_len % 4));
			TPD_INFO("fix w_len = 0x%X\n", w_len);
		}
...
static int ilitek_tddi_fw_iram_upgrade(u8 *pfw)
{
    ...
    for (i = 0; i < ARRAY_SIZE(fbi); i++) {
        if (fbi[i].mode == mode && fbi[i].len != 0) {
            ...
            dma = 0;
            ret = host_download_dma_check(fbi[i].mem_start, fbi[i].len - crc_len, &dma);
			if (mode != GESTURE) {
				ilitek_tddi_fw_iram_read(crc_temp, (fbi[i].mem_start + fbi[i].len - crc_len), (fbi[i].mem_start + fbi[i].len - 1));
				iram_crc = crc_temp[0] << 24 | crc_temp[1] << 16 |crc_temp[2] << 8 | crc_temp[3];
				if (iram_crc != dma)
					iram_crc_err = true;
			}
			TPD_INFO("%s CRC is %s hex(0x%X) : dma(0x%X) : iram(0x%X), calculation len is 0x%X\n",
				fbi[i].name,((crc != dma)||(iram_crc_err)) ? "Invalid !" : "Correct !", crc, dma, iram_crc, fbi[i].len - crc_len);

             if(ret < 0 || crc != dma || iram_crc_err) {
                TPD_DEBUG("fw print iram data\n");
