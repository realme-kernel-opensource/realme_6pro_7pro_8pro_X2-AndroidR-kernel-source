...
static ssize_t ilitek_proc_get_delta_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
    ...
    TPD_INFO("cat proc/touchpanel/debug_info/delta \n");
...
static ssize_t ilitek_proc_fw_get_raw_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
    ...
    TPD_INFO("cat proc/touchpanel/debug_info/baseline \n");
...
static ssize_t ilitek_node_change_list_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "[Patch] add slave mode YangYang 2020/06/11\n");
...
/*Jianghao, 2020/05/26, add for modify spi speed*/
        ...
        TPD_INFO("enter deep sleep mode ,gesture disabled \n");
    } else if (strcmp(cmd, "gesturemode") == 0) {
        temp[0] = 0xF6;
        temp[1] = 0x0A;
    ...
    }  else if (strcmp(cmd, "gesturemode1") == 0) {
        if(ilitek_tddi_ic_func_ctrl("lpwg", 0x2)< 0)
            ipio_err("write gesture flag1 failed \n");
    } else if (strcmp(cmd, "setspispeed") == 0) {
        int speed = 0;
        speed = data[1];
        TPD_INFO("setspeed = %d\n", speed);
        idev->spi->max_speed_hz = speed;
        TPD_INFO("idev->spi->max_speed_hz = %d",idev->spi->max_speed_hz);
    } else if (strcmp(cmd, "getspispeed") == 0) {
        TPD_INFO("idev->spi->max_speed_hz = %d",idev->spi->max_speed_hz);
#endif
    ...
    } else if (strcmp(cmd, "checkirq") == 0) {
        //TPD_INFO("checkirq\n");
        idev->need_judge_irq_throw = true;
        ilitek_check_wake_up_state(5000);
	...
	} else if (strcmp(cmd, "siow") == 0) {
		int w_len = 0;
		w_len = data[1];
		TPD_INFO("spi slave w len = %d\n", w_len);
		for (i = 0; i < w_len; i++) {
			temp[i] = data[2 + i];
			TPD_INFO("write[%d] = %x\n", i, temp[i]);
		}
		core_spi_slave_mode_write(temp, w_len);
	} else if (strcmp(cmd, "sior") == 0) {
		int r_len = 0;
		r_len = data[1];
		TPD_INFO("spi slave r len = %d\n", r_len);
		core_spi_slave_mode_read(temp, r_len);
		for (i = 0; i < r_len; i++)
			TPD_INFO("read[%d] = %x\n", i, temp[i]);
	} else if (strcmp(cmd, "siowr") == 0) {
		int r_len = 0, w_len = 0;
		r_len = data[1];
		w_len = data[2];
		TPD_INFO("spi slave r len = %d, w len = %d\n", r_len, w_len);
		for (i = 0; i < w_len; i++) {
			temp[i] = data[3 + i];
			TPD_INFO("write[%d] = %x\n", i, temp[i]);
		}
...
/*dongfeiju, 2020/06/11, add for slave mode*/
    case ILITEK_IOCTL_TP_PANEL_INFO:
        ...
        TPD_INFO("ioctl: get panel resolution %d x %d\n",id_to_user[0],id_to_user[1]);
    ...
    case ILITEK_IOCTL_TP_INFO:
        ...
        TPD_INFO("ioctl: get panel resolution %d,%d,%d,%d,%d,%d,%d,%d\n",
            id_to_user[0],id_to_user[1],id_to_user[2],id_to_user[3],
            id_to_user[4],id_to_user[5],id_to_user[6],id_to_user[7]);
...
/*dongfeiju, 2020/06/11, add for slave mode*/
	case ILITEK_IOCTL_WRAPPER_RW:
		TPD_DEBUG("ioctl: slave wrapper rw\n");
		...
		wrap_rlen = (szBuf[0] << 8) | szBuf[1];
		wrap_wlen = (szBuf[2] << 8) | szBuf[3];

		TPD_DEBUG("wrap_rlen = %d\n", wrap_rlen);
		TPD_DEBUG("wrap_wlen = %d\n", wrap_wlen);
		...
		ilitek_thp_send_cmd(szBuf + 4, wrap_wlen, wrap_rbuf, wrap_rlen);

		ilitek_dump_data(szBuf + 4, 8, wrap_wlen, 16, "wrap_wbuf:");
		ilitek_dump_data(wrap_rbuf, 8, wrap_rlen, 16, "wrap_rbuf:");

		if ((wrap_rlen > 0) && copy_to_user((u8 *) arg, wrap_rbuf, wrap_rlen)) {
			ipio_err("Failed to copy driver ver to user space\n");
			ret = -ENOTTY;
		}
//#endif
