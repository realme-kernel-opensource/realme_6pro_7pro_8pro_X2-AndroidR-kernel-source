...
inline void oppo_sync_saupwk_event(unsigned int type, unsigned int code, int value)
{
...
static const struct file_operations oppo_sau_pwknum_fops = {
    .owner   = THIS_MODULE,
...
oppo_saupwk_during_init(void)
{
    ...
    pentry = proc_create("sau_pwknum", 0, NULL, &oppo_sau_pwknum_fops);
    if(!pentry){
