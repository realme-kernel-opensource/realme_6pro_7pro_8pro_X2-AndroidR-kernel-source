	...
	const char *keystring, unsigned long long flag)
{
	...
	hlist_for_each_entry(pnode, phead, hnode) {
		if (pnode) {
			pr_info("ACM: acm_hsearch_with_flag:0x%llx 0x%llx %s %s\n",
				pnode->afp.flag, flag, pnode->afp.pkgname, keystring);
	...
	const char *keystring)
{
	...
	hlist_for_each_entry(pnode, phead, hnode) {
		if (pnode) {
			pr_info("ACM: acm_hsearch:0x%llx %s %s\n", pnode->afp.flag,
				pnode->afp.pkgname, keystring);
			if (!strcmp(pnode->afp.pkgname, keystring)) {
				break;
			}
		}
...
static int do_cmd_add_pkg(unsigned long args)
{
	...
	if (result) {
		if (result->afp.flag != hnode->afp.flag) {
			...
			pr_info("ACM: Updata the flag of package\n");
		} else {
			pr_info("ACM: Package is already in the white list!\n");
		}
		err = ACM_SUCCESS;
		goto do_cmd_add_ret;
	}
	...
	list_for_each_entry(dir_node, &acm_dir_list.head, lnode) {
		if (strncmp(fwk_dir->dir, dir_node->afd.dir, ACM_DIR_MAX - 1) == 0) {
			if (dir_node->afd.flag != fwk_dir->flag) {
				...
				pr_err("ACM: Updata the flag of Dir\n");
			} else {
				pr_err("ACM: Dir is already in the dir list!\n");
			}
			goto add_dir_ret;
		}
	}
...
static int do_cmd_add_nomediadir(unsigned long args)
{
	...
	list_for_each_entry(dir_node, &acm_nomediadir_list.head, lnode) {
		if (strncasecmp(fwk_ndir->dir, dir_node->afnd.dir, ACM_DIR_MAX - 1) == 0) {
			pr_err("ACM: Nomedia dir is already in the dir list!\n");
			goto add_dir_ret;
		}
	}
...
static int do_cmd_search_nomediadir(unsigned long args)
{
	...
	pr_info("ACM: will %s %s\n", __func__, fwk_ndir->dir);
	...
	pr_info("ACM: %s %s %s\n", __func__, err ? "can find" : "cannot find",
		   fwk_ndir->dir);
	...
	struct dentry *dentry, int file_type, int op)
{
	...
	list_for_each_entry_safe(dir_node, n, &acm_dir_list.head, lnode) {
		if (strstr(dentry_path, dir_node->afd.dir) != NULL) {
			...
			pr_info("ACM: %s get %s 0x%llx\n", __func__, dentry_path,
				dir_flag);
		}
	}
	...
	if (dir_flag == 0) {
		...
		if (op == ACM_OPT_RENAME || op == ACM_OPT_RENAME2) {
			return DEL_ALLOWED;
		}
...
int acm_search(struct dentry *dentry, int file_type, int op)
{
	...
	*/
	...
	pr_info("ACM: %s taskuid=%u pkg=%s\n", __func__, taskuid, pkgname);

	if ((acm_opstat(ACM_FLAG_DEL | ACM_FLAG_LOGGING))
		&& (op == ACM_OPT_UNLINK || op == ACM_OPT_RMDIR
			|| op == ACM_OPT_RENAME || op == ACM_OPT_RENAME2)) {
		...
		&& (op == ACM_OPT_CRTEAT || op == ACM_OPT_MKNOD)) {
...
static void set_logging_uevent_env(struct acm_lnode *node)
{
	...
	/* for test */
	pr_info("ACM: %s %s %s %s %s %s %s %s\n",
		logging_env.envp[0], logging_env.envp[1], logging_env.envp[2],
		logging_env.envp[3], logging_env.envp[4], logging_env.envp[5],
...
static int acm_chr_dev_init(void)
{
	...
	pr_info("ACM: Initialize acm character device succeed!\n");
	return err;
